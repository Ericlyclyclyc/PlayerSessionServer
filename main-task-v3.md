...
2. AccessToken管理
   ...
   • 使Token失效：提供API使指定的Token立即失效，用于安全措施（如检测到异常活动、强制下线等）。
...
4. 使Token失效
   ...
   • 请求体：{"username": "string", "uuid": "string", "token": "string"}

   • 响应：使指定Token立即失效，返回操作结果。
...
• TokenService：生成Token、验证Token、使Token失效、管理超时（通过定时任务或退出时计算时间差标记过期）。
...
• Token强制失效：管理员或系统可以通过API使特定Token立即失效，用于安全措施。
